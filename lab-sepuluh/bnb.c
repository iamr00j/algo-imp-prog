...
void branch_and_bound(int *final_sol)
{
  ...
  for(int i=1; i<=NITEMS; i++)
    solution.solution_vec[i] = 0;
  solution.fixed = 0;
  
  frac_bound(&solution, solution.fixed);
  int current_best = solution.val;
  double upper_bound = solution.bound;
  insert(solution);
  
  do{
    struc_sol child1 = removeMax();
    struc_sol child2;
    copy_array(child1.solution_vec, child2.solution_vec);
    child2.val = child1.val;
    child2.fixed = child1.fixed;
    child2.bound = child1.bound;
    
    //child1 with 1 added, child2 with 0 added
    child1.solution_vec[child1.fixed + 1] = 1;
    child1.fixed = child1.fixed +1;
    
    child2.solution_vec[child2.fixed + 1] = 1;
    child2.fixed = child2.fixed + 1;
    
    int weight1 = 0;
    for(int i =1; i<=child1.fixed; i++){
      if(child1.solution_vec[i]==1)
        weight1 += item_weights[temp_indexes[i]];
    }
    ...
    if(weight1<= Capacity){
      ...
      if(child1.val >current_best){
        ...
        final_sol[child1.fixed] = child1.solution_vec[child1.fixed];
        upper_bound = child1.bound;
      }
      insert(child1);
    }
    
    int weight2 = 0;
    for(int i =1; i<=child2.fixed; i++){
      if(child2.solution_vec[i]==1)
        weight2 += item_weights[temp_indexes[i]];
    }
    ...
    if(weight2<= Capacity){
      ...
      if(child2.val >current_best){
        ...
        final_sol[child2.fixed] = child2.solution_vec[child2.fixed];
        upper_bound = child2.bound;
      }
      insert(child2);
    }
  } while(QueueSize > 0 && upper_bound > current_best);
